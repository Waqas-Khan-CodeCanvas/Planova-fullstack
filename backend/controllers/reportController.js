...
const exportTasksReport = async (req , res) => {
    try {
        const tasks = await Task.find().population("assignedTo" , "name email");
        ...
        { header: "Title", key: "tittle", width: 30},
        { header: "Description", key: "description", width: 50},
        { header: "Priority", key: "priority", width: 15},
        { header: "Status", key: "status", width: 20 },
        { header: "Due Date", key: "duedate", width: 20},
        { header: "Assigned To", key: "assignment", width: 30},    
     ] ;
     ...
     res.setHeader(
        "Content-Disposition",
        'attachement; filename="tasks-report.xlsx"'
     );
...
const exportUsersReport = async (req, res) => {
    try {
        ...
        const userTasks = await Task.find().population(
            "assignedTo",
            "name email _id"
        );
